...
const TrangChu = ({ navigation }) => {
  ...
  const renderCayTrong = ({ item }) => {
    return (
      ...
      <View style={{ margin: 20 }}>
        ...
        {item.status ? <Text>Đã hết hàng</Text> : <Text>Còn hàng</Text>}
  ...
  return (
    <SafeAreaView style={styles.conner}>
      <ScrollView>
        <Image source={require('../Img/banner.jpg')} style={{ width: '100%',height: 150, marginTop: 115, marginBottom: 20 }} />
        <Text style={[styles.txt, { position: 'absolute', top: 20 }]}>Planta - toả sáng{'\n'}không gian nhà bạn{'\n'}
        ...
        <TouchableOpacity onPress={() => navigation.navigate('xemThemCt', { data: data })}>
          <Text style={{ textAlign: 'right', margin: 20, textDecorationLine: 'underline', fontSize: 20, color: 'black' }}>Xem thêm sản phẩm</Text>
        </TouchableOpacity>
        ...
        <Image style={{width:200,margin:20}} source={require('../Img/them.jpg')}/>
      </ScrollView>
    </SafeAreaView>
  )
}
